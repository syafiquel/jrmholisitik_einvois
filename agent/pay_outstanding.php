<?php
 ...
 if ($result->num_rows > 0) {
   while($row = $result->fetch_assoc()) {
     ...
     if($row['status'] == 'outstanding' && $row['payment_approval_status'] != 'pending'){
       $fullSettlementAmount += $row['outstanding_amount'];
       $fullSettlementIDs[] = $row['order_id'];
     }
     $allOutstanding[] = $row['order_id'];
   }
 }
...
if (isset($_POST['pay_outstanding'])) {
    ...
    if (isset($_POST['order_id'])) {
      ...
      $selectedOrders = array_unique($selectedOrders);
    ...
    $payment_ref = $payment_ref .'-'. $created_at;
    ...
    if (isset($paymentDetails)) {
      ...
      $payment_desc = 'Credit Terms Settlement';
      $totalPaid = $fullSettlementAmount;
      $fullSettlementIDs = array_unique($fullSettlementIDs);

      $selectedIds = implode(',', $fullSettlementIDs);
    }
    Credit::updateSettlementRef($selectedIds, $payment_ref);

    if ($totalPaid == 0) {
      $_SESSION['success'] = "Action Failed. Please select the right outstanding order.";
      header($url);
      exit;
    }
    // echo $totalPaid;
    ...
    if ($_POST['payment_method'] === 'billplz') {
      ...
      $totalPaid = $user['no_tel'] == '0123456789' ? '1':$totalPaid;
      $parameter = array(
        ...
        if(empty($errors_uploading)==true){
          ...
          if(mysqli_query($db, $sql))
          {
            ...
            if ($settlement_type == 'Partial') {
              ...
              AND payment_approval_status != 'pending'
              AND payment_approval_status != 'pending'
              ";
            }
            mysqli_query($db, $sql);
          ...
          <div class="mt-4">
            <form method="post" enctype="multipart/form-data">
              ...
              <?php elseif ($user['credit_used']): ?>
                ...
                <button <?php echo !count($outstanding) ? 'disabled':'' ?> type="submit" class="btn btn-primary mt-2"style="width: 100%;" name="pay_outstanding">
