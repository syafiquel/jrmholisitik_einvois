...
$page_name = "Credit Terms Settlement Rejected ";
$url = "settlement_rejected.php";
...
if (isset($_POST['approve_payment'])) {
  ...
  $sql = "UPDATE credit_settlement SET approval_status = '$outstanding_status' , paid_at = $approval_status WHERE settlement_ref = '$settlement_ref'";
  ...
  if (mysqli_query($db,$sql)) {
    if ($_POST['status'] == 'approve') {
      ...
      if ($credit_info['type'] == 'Full') {
        $where_clause = "user_id = '$user_id'";
      }
      if ($credit_info['type'] == 'Partial') {
        $where_clause = "settlement_ref = '$settlement_ref'";
      }

      $sql = "UPDATE user SET
      credit_used = credit_used - {$credit_info['amount']}
      WHERE id = '$user_id'";
      mysqli_query($db, $sql);

      $sql = "UPDATE credit_terms SET
      status = 'completed',
      payment_approval_status = 'completed',
      settled_at = now()
      WHERE $where_clause";
      mysqli_query($db, $sql);
...
FROM credit_settlement ct
 ...
 WHERE ct.approval_status = 'rejected'
 ORDER BY ct.id DESC ";
// echo $sql;
// exit;
$result_c = $db->query($sql);
                     ...
                     <?php
                      ...
                      ?>
                       <tr>
                         ...
                         <td class="text-center"><?php echo $row['settlement_ref'] ?></td>
